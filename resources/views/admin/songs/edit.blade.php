        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form action="{{ route('admin.songs.update', $song) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="wallpaper_id" id="wallpaper_id" 
                            ...
                            @foreach($wallpapers as $wallpaper)
                                <option value="{{ $wallpaper->id }}" {{ (old('wallpaper_id', $song->wallpaper_id) == $wallpaper->id) ? 'selected' : '' }}>
                                    {{ $wallpaper->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Current Audio File</label>
                        @if($song->file_path)
                            <audio controls class="w-full mb-2">
                                <source src="{{ Storage::url($song->file_path) }}" type="audio/mpeg">
                                Your browser does not support the audio element.
                            </audio>
                            <p class="text-sm text-gray-500">{{ basename($song->file_path) }}</p>
                        @else
                    ...
                    <div class="mb-4">
                        <label for="audio_file" class="block text-sm font-medium text-gray-700">New Audio File (Leave empty to keep current)</label>
